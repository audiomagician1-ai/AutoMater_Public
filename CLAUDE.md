...
> 最后更新: 2026-03-05 | 版本: v33.1 | **由代码实际盘点生成，非手工维护**

## 1. PRIME DIRECTIVE

**当前阶段**: v33.1 — Session-Driven并发全面加固(Worker异常恢复+PM输出Rescue+启动态重置)
...
**版本**: v33.1 (Session-Driven并发全面加固 + PM输出Rescue + Worker异常恢复 + 启动态重置 + 130工具)
...
- [x] **v33.1 Session-Driven并发全面加固** — (1) Worker异常恢复: spawnSessionWorker的promise.catch自动释放feature锁+发射session_failed事件+重新调度, 消除zombie feature风险 (2) PM输出Rescue: pm-phase解析失败时用mini模型从终止总结中提取Feature JSON, 解决error_loop终止后自然语言覆盖finalText导致的解析失败 (3) 启动态重置: 应用重启时developing/initializing/analyzing三种活跃状态统一重置为paused(原来只处理analyzing) (4) react-generic-loop streaming: PM/Architect的react loop补充stream事件输出到UI
