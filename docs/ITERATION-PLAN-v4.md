# AgentForge v4.x 迭代计划 — 工作流重构与质量体系升级

> 撰写时间: 2026-03-01  
> 基线版本: v3.1.1 (commit `a027149`)  
> 目标: 将 AgentForge 从"原型验证级"升级为"可舒适使用的专业工具"
...
| `src/pages/LogsPage.tsx` | 初始加载时 `window.agentforge.project.getLogs()` 填充历史；支持分页加载更早日志；增加搜索/过滤 |
...
4. 项目 memory (`.agentforge/project-memory.md`)
...
| 文档浏览器 | 新页面或面板: 树状展示 `.agentforge/docs/` 下所有文档，点击查看 Markdown 渲染，显示版本历史 |
