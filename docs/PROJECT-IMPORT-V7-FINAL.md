...
### Phase A: 最小验证 (3-5h) — ✅ **已完成 (2026-03-02)**

- [x] **A1**: 在现有 `importProject()` 基础上添加串行探测 (3 个策略: Entry + Module + Data Model) → 实际实现了 6 类并行探针
- [x] **A2**: ProbeReport 写入 `.automater/analysis/probes/`
- [x] **A3**: Phase 2 修改 prompt 注入 ProbeReport
- [x] **A4**: 在 AgentForge 自身 + 一个外部项目上对比 v6.0 vs v7.0 的 ARCHITECTURE.md → 待实际导入验证

**实际交付**:
- Phase A + B + C1/C4 一次性实施 (跳过 Phase B 的分步迭代)
- 新增 12 个文件: probe-types.ts, probe-orchestrator.ts, probes/{base,entry,module,api-boundary,data-model,config-infra,smell,index}.ts
- 增强 code-graph.ts: +detectCommunities() +getHubFiles() +buildProjectProfile()
- 重写 project-importer.ts: v6.0 (2步) → v7.0 (3阶段: Scan→Probe→Fuse)
- 产出: module-graph.json + ARCHITECTURE.md + KNOWN-ISSUES.md + probe-reports/ + import-stats.json
- 测试: 29 新测试 (probe-system.test.ts), 全量 734/734 通过, tsc 0 error

**验证标准**:
- ARCHITECTURE.md 包含的具体函数名/类型名数量 ↑ 50%+ → 待实际导入对比
- 模块摘要的 publicAPI 字段非空率 > 80% → 由 module-graph.json 节点 publicAPI 字段保证
- 人工判断: 关键数据流描述是否正确 → 待实际导入验证

### Phase B: 并行 + 完整策略 (4-6h) — ✅ **已完成 (与 Phase A 合并实施)**

- [x] **B1**: 提取 `probe-orchestrator.ts` + `probe-types.ts`
- [x] **B2**: 并行执行 (concurrency 控制 + Promise.race 调度)
- [x] **B3**: 添加 API Boundary + Config/Infra + Smell 三个探针
- [x] **B4**: 增强 `code-graph.ts` — `detectCommunities()` + `getHubFiles()`
- [x] **B5**: `buildProjectProfile()` 驱动探针数量
- [x] **B6**: ProbeReport JSON Schema + output-parser 验证 → extractJSON + structured prompt

### Phase C: 结构化图 + 下游集成 (4-6h) — 🔶 **部分完成**

- [x] **C1**: Phase 2 输出 `module-graph.json` (RPG 风格)
- [ ] **C2**: `context-collector.ts` 使用 module-graph 做图引导的文件选择
- [ ] **C3**: orchestrator 注入 known-issues.md 到 Developer 上下文
- [x] **C4**: 丰富 skeleton.json — publicAPI, keyTypes, 实际依赖关系
