...
function ensureAgentsMd(workspacePath: string, wish: string) {
  ...
  const content = [
    ...
    `- 新增文件前先搜索是否已有类似功能`,
    ``,
    `## ⚡ 精准读写纪律 (节省 token、提高效率)`,
    `- **先搜后读**: 修改文件前必须先 search_files/code_search 定位目标行号`,
    `- **小范围精读**: read_file 必须带 offset + limit 参数，通常只读 30-80 行`,
    `- **禁止盲读**: 不要不带 offset 地 read_file 读取整文件`,
    `- **善用索引**: code_graph_query 查依赖关系，repo_map 查符号索引`,
    ``,
    `## 常用命令`,
