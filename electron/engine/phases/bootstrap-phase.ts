...
export async function phaseEnvironmentBootstrap(
  ...
  if (hasPackageJson) {
    ...
    if (!hasNodeModules) {
      ...
      sendToUI(win, 'agent:log', {
        projectId,
        agentId: 'system',
        content: `  📦 安装 Node.js 依赖: ${installCmd.split(' 2>&1')[0]}`,
      });
      try {
    ...
    if (!hasVenv && hasRequirements) {
      ...
      try {
        await execAsync('pip install -r requirements.txt 2>&1', {
          cwd: workspacePath,
          encoding: 'utf-8',
          timeout: 180_000,
        });
  ...
  if (secrets.length > 0) {
    const envLines: string[] = [
      '# Auto-generated by AutoMater — DO NOT COMMIT',
      `# Project: ${projectId}`,
    ...
    if (envLines.length > 4) {
      ...
      sendToUI(win, 'agent:log', {
        projectId,
        agentId: 'system',
        content: `  🔑 ${secrets.length} 个密钥已注入 .env.local`,
      });
    }
  }
  ...
  if (gitConfig.mode === 'github' && gitConfig.githubRepo && gitConfig.githubToken) {
    try {
      ...
      sendToUI(win, 'agent:log', {
        projectId,
        agentId: 'system',
        content: `  🔗 Git 远程已配置: ${gitConfig.githubRepo}`,
      });
    } catch (_err) {
  ...
  if (hasSecret(projectId, 'github_token')) {
    ...
    if (repo && token) {
      ...
      sendToUI(win, 'agent:log', {
        projectId,
        agentId: 'system',
        content: `  ${testResult.success ? '✅' : '❌'} GitHub: ${testResult.message}`,
      });
    }
  }
    ...
    if (token && projectRef) {
      try {
        ...
        sendToUI(win, 'agent:log', {
          projectId,
          agentId: 'system',
          content: `  ${testResult.success ? '✅' : '❌'} Supabase: ${testResult.message}`,
        });
      } catch (err) {
        ...
        sendToUI(win, 'agent:log', {
          projectId,
          agentId: 'system',
          content: `  ❌ Supabase: 连接测试异常 (${err instanceof Error ? err.message : String(err)})`,
        });
      }
    } else {
      result.credentialsValid['supabase'] = false;
      sendToUI(win, 'agent:log', {
        projectId,
        agentId: 'system',
        content: `  ❌ Supabase: 缺少 ${!token ? 'access_token' : 'project_ref'}`,
      });
    }
  }
    ...
    if (token) {
      try {
        ...
        sendToUI(win, 'agent:log', {
          projectId,
          agentId: 'system',
          content: `  ${testResult.success ? '✅' : '❌'} Cloudflare: ${testResult.message}`,
        });
      } catch (err) {
        ...
        sendToUI(win, 'agent:log', {
          projectId,
          agentId: 'system',
          content: `  ❌ Cloudflare: 连接测试异常 (${err instanceof Error ? err.message : String(err)})`,
        });
      }
    } else {
      result.credentialsValid['cloudflare'] = false;
