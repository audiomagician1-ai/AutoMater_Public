...
export async function phasePMAnalysis(
  ...
  try {
    if (signal.aborted) return _fail('中止');
  ...
  if (!pmGate.passed) {
    ...
    db.prepare("UPDATE projects SET status = 'error', updated_at = datetime('now') WHERE id = ?").run(projectId);
    return _fail(`PM→Architect 门控未通过: ${pmGate.reason}`);
  }
  ...
  const pmResult: PMPhaseResult = {
    ...makePhaseResult('pm_analysis', 'success', `PM 分析完成 (${features.length} Features)`, startTime, {
      artifacts: { featureIds: features.map(f => f.id) },
    }),
    features,
  };
  return pmResult;
...
export async function phaseIncrementalPM(
  ...
  try {
    ...
    if (signal.aborted) return _failInc('中止');
    const errMsg = err instanceof Error ? err.message : String(err);
    sendToUI(win, 'agent:log', { projectId, agentId: pmId, content: `❌ 增量分析失败: ${errMsg}` });
    return _failInc(`增量分析失败: ${errMsg.slice(0, 100)}`);
  }
}
...
export async function phasePMAcceptance(
  ...
  if (qaPassed.length === 0) {
    ...
    return makePhaseResult('pm_acceptance', 'skipped', '没有 Feature 需要 PM 验收', startTime);
  }
  ...
  for (let bi = 0; bi < accBatches.length; bi++) {
    if (signal.aborted) return makePhaseResult('pm_acceptance', 'failure', '中止', startTime);
  ...
  return makePhaseResult('pm_acceptance', 'success', `PM 验收完成 (${qaPassed.length} features)`, startTime);
}
