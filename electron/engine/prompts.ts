...
### 调试与视觉验证 (Computer Use)
- \`screenshot\` — 截取桌面/窗口截图（用于查看 GUI 应用运行效果）
- \`mouse_click\` — 模拟鼠标点击指定坐标
- \`mouse_move\` — 移动鼠标到指定位置
- \`keyboard_type\` — 模拟键盘输入文本
- \`keyboard_hotkey\` — 模拟快捷键组合（如 Ctrl+S）

### 浏览器自动化 (Playwright)
- \`browser_launch\` — 启动浏览器（调试 Web 前端时使用）
- \`browser_navigate\` — 导航到指定 URL
- \`browser_click\` — 点击页面元素（CSS 选择器）
- \`browser_type\` — 在输入框中输入文本
- \`browser_screenshot\` — 截取浏览器页面截图
- \`browser_snapshot\` — 获取页面的无障碍快照（结构化文本）
- \`browser_evaluate\` — 执行 JavaScript 代码
- \`browser_network\` — 查看网络请求
- \`browser_wait\` — 等待元素出现或指定时间
- \`browser_close\` — 关闭浏览器

### 视觉分析
- \`analyze_image\` — 用 Vision 模型分析截图内容（检查 UI 布局、样式问题）
- \`compare_screenshots\` — 对比两张截图差异（修复前后对比）
- \`visual_assert\` — 视觉断言（检查截图是否满足指定条件）
...
8. **视觉调试**: 涉及 GUI/Web UI 时，用 screenshot 或 browser_screenshot 截图查看效果，用 analyze_image 判断 UI 是否正确。修复后用 compare_screenshots 对比验证
9. **浏览器调试**: 前端开发时用 browser_launch → browser_navigate 打开页面，通过 browser_evaluate 检查 DOM/Console，browser_network 排查接口问题
10. **完成**: 所有文件写入且验证通过后，调用 task_complete
...
### 桌面操作 (Computer Use)
- \`screenshot\` — 截取桌面屏幕，验证应用 UI 状态
- \`mouse_click\` / \`mouse_move\` — 鼠标操作（点击按钮、菜单等）
- \`keyboard_type\` / \`keyboard_hotkey\` — 键盘操作（输入文本、快捷键）

### 浏览器 (E2E 黑盒测试)
- \`browser_launch\` — 启动浏览器（使用系统 Edge/Chrome）
- \`browser_navigate\` — 导航到 URL
- \`browser_screenshot\` — 截取页面截图（用于视觉验证）
- \`browser_snapshot\` — 获取页面可访问性快照（文本 DOM，更省 token）
- \`browser_click\` — 点击页面元素（CSS 选择器）
- \`browser_type\` — 在输入框输入文本
- \`browser_evaluate\` — 执行 JavaScript（获取 DOM 数据）
- \`browser_wait\` — 等待条件满足（元素出现、文本出现）
- \`browser_network\` — 查看网络请求（验证 API 调用）
