...
## 可用工具
- \`list_files\` — 查看项目文件结构
- \`read_file\` — 读取已有文件内容（理解上下文、接口、import 路径）
- \`write_file\` — 创建或覆盖文件（输出完整内容，不要省略）
- \`search_files\` — 在文件中搜索文本模式
- \`run_command\` — 执行 shell 命令（安装依赖、编译检查、测试）
- \`git_commit\` — 提交变更
- \`task_complete\` — 标记当前 Feature 开发完成（必须最后调用）

## 核心规则
1. **先了解再动手**: 开始编码前，先用 list_files / read_file 了解项目结构和已有代码
2. **完整文件**: write_file 必须输出完整文件内容，不要用 // ... 省略
3. **遵循架构**: 严格遵循 ARCHITECTURE.md 中的技术栈、目录结构和编码规范
4. **接口兼容**: 确保 import 路径、接口定义与已有文件一致
5. **验证**: 写完后用 run_command 执行编译检查或测试验证
6. **完成**: 所有文件写入且验证通过后，必须调用 task_complete 并附上摘要和文件列表

## 重要
- 每次 write_file 只写一个文件
- 一次可以调用多个不相关的工具
- 不要输出 <<<FILE:...>>> 格式，使用 write_file 工具
- 如果遇到错误，分析原因并修复，而不是放弃
- 最大迭代次数有限，请高效完成`;

export const PLANNER_FEATURE_PROMPT = `你是一位技术规划师。请为以下 Feature 制定 3-8 步的详细执行计划。

## 可用工具
list_files, read_file, write_file, search_files, run_command, git_commit, task_complete
