...
// v30.2: 拆分子模块 — re-export 保持向后兼容
export type { ReactResult, MessageTokenBreakdown, ReactIterationState, AgentReactState } from './react-state';
export { getAgentReactStates, cacheAgentReactState, getContextSnapshots, cacheContextSnapshot } from './react-state';
export type { GenericReactConfig, GenericReactResult } from './react-generic-loop';
export { reactAgentLoop } from './react-generic-loop';

import { cacheAgentReactState, cacheContextSnapshot } from './react-state';
...
// Token estimation + message breakdown → react-helpers.ts

// Termination summary → react-helpers.ts
...
// Feature domain inference → react-compression.ts
// Message compression → react-compression.ts
// Generic ReAct Agent Loop → react-generic-loop.ts
