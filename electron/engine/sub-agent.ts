...
interface MiniTool {
  name: string;
  execute: (args: Record<string, any>, ctx: ToolContext) => string;
}
...
const MINI_TOOLS: MiniTool[] = [
  {
    name: 'read_file',
    execute(args, ctx) {
      const content = readWorkspaceFile(ctx.workspacePath, args.path);
  ...
  },
  {
    name: 'list_files',
    execute(args, ctx) {
      const tree = readDirectoryTree(ctx.workspacePath, args.directory || '', args.max_depth ?? 3);
      const format = (nodes: any[], indent = ''): string => {
        return nodes.map(n => {
  ...
  },
  {
    name: 'memory_read',
    execute(args, ctx) {
      const mem = readMemoryForRole(ctx.workspacePath, args.role || 'developer');
...
const RESEARCHER_TOOLS_FOR_LLM = [
  {
    type: 'function',
    function: {
      name: 'read_file',
...
## 规则
- 你只能读取和搜索文件，不能修改任何内容
- 先用 list_files 了解结构，再用 read_file / search_files 深入
...
export async function runResearcher(
  ...
  callLLMFn: (
    ...
    tools: any[],
  ) => Promise<{ message: any; inputTokens: number; outputTokens: number }>,
  signal?: AbortSignal,
  ...
  for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
    ...
    try {
      const result = await callLLMFn(messages, RESEARCHER_TOOLS_FOR_LLM);
      ...
      for (const tc of msg.tool_calls) {
        ...
        if (tc.function.name === 'done') {
          return {
            success: true,
            conclusion: toolArgs.conclusion || '研究完成',
            filesRead,
            inputTokens: totalIn,
            outputTokens: totalOut,
          };
        ...
        if (tool) {
          try {
            output = tool.execute(toolArgs, ctx);
