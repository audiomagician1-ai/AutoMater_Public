...
export async function executeToolAsyncRaw(call: ToolCall, ctx: ToolContext): Promise<ToolResult> {
  ...
  if (call.name === 'github_close_issue') {
    ...
    return ok
      ...
      : {
          success: false,
          output: `关闭 Issue #${call.arguments.issue_number} 失败 (可能未配置 GitHub 模式)`,
          action: 'github',
        };
  }
  ...
  if (call.name === 'search_images') {
    ...
    const lines = result.images.map(
      (
        img: { url: string; thumbnailUrl: string; title: string; source: string; width?: number; height?: number },
        i: number,
      ) =>
  ...
  if (call.name === 'deep_research') {
    ...
    const settings: AppSettings = settingsRow
      ? safeJsonParse<AppSettings>(settingsRow.value, {} as AppSettings)
      : ({} as AppSettings);
  ...
  if (call.name === 'browser_launch') {
    ...
    return {
      success: result.success,
      output: result.success ? '浏览器已启动' : `启动失败: ${result.error}`,
      action: 'computer',
    };
  }
  ...
  if (call.name === 'browser_snapshot') {
    ...
    return {
      success: result.success,
      output: result.success ? result.content : `快照失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_click') {
    ...
    return {
      success: result.success,
      output: result.success ? `已点击: ${call.arguments.selector}` : `点击失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_type') {
    ...
    return {
      ...
      output: result.success
        ? `已输入 ${call.arguments.text.length} 字符到 ${call.arguments.selector}`
        : `输入失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_evaluate') {
    ...
    return {
      success: result.success,
      output: result.success ? result.result : `执行失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_wait') {
    ...
    return {
      success: result.success,
      output: result.success ? '等待条件已满足' : `等待超时: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_network') {
    ...
    return {
      success: result.success,
      output: result.success ? result.requests : `网络监听失败: ${result.error}`,
      action: 'computer',
    };
  }
  ...
  if (call.name === 'analyze_image') {
    ...
    if (!base64)
      return {
        success: false,
        output: `未找到标签为 "${call.arguments.image_label || 'latest'}" 的截图。请先截图。`,
        action: 'computer',
      };
    const result = await analyzeImage(base64, call.arguments.question, ctx.callVision);
    return {
      success: result.success,
      output: result.success ? result.analysis : `分析失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'compare_screenshots') {
    ...
    if (!beforeBase64)
      return { success: false, output: `未找到 "before" 截图: "${call.arguments.before_label}"`, action: 'computer' };
    if (!afterBase64)
      return {
        success: false,
        output: `未找到 "after" 截图: "${call.arguments.after_label || 'latest'}"`,
        action: 'computer',
      };
  ...
  if (call.name === 'visual_assert') {
    ...
    if (!base64)
      return {
        success: false,
        output: `未找到标签为 "${call.arguments.image_label || 'latest'}" 的截图`,
        action: 'computer',
      };
  ...
  if (call.name === 'browser_hover') {
    ...
    return {
      success: result.success,
      output: result.success ? `已悬停: ${call.arguments.selector}` : `悬停失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_select_option') {
    ...
    return {
      success: result.success,
      output: result.success ? `已选择: ${(result.selected || []).join(', ')}` : `选择失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_press_key') {
    ...
    return {
      success: result.success,
      output: result.success ? `已按键: ${call.arguments.key}` : `按键失败: ${result.error}`,
      action: 'computer',
    };
  }
  ...
  if (call.name === 'browser_drag') {
    ...
    return {
      success: result.success,
      output: result.success
        ? `已拖放: ${call.arguments.source_selector} → ${call.arguments.target_selector}`
        : `拖放失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_tabs') {
    ...
    return {
      success: result.success,
      output: result.success ? `标签页操作完成: ${call.arguments.action}` : `操作失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_file_upload') {
    ...
    return {
      success: result.success,
      output: result.success
        ? `已上传 ${(call.arguments.file_paths || []).length} 个文件`
        : `上传失败: ${result.error}`,
      action: 'computer',
    };
  }
  if (call.name === 'browser_console') {
    ...
    return {
      success: true,
      output: result.messages?.length ? result.messages.join('\n') : '(无控制台消息)',
      action: 'computer',
    };
  }
  ...
  if (call.name === 'sandbox_init') {
    ...
    const config = presetConfig
      ? {
          ...presetConfig,
          mountWorkspace: call.arguments.mount_workspace,
          hostWorkspacePath: ctx.workspacePath,
      ...
      : {
          ...
          hostWorkspacePath: ctx.workspacePath,
          env: call.arguments.env,
          memoryLimit: call.arguments.memory_limit || '1g',
          workDir: '/workspace',
        };
    ...
    return result.success
      ? {
          success: true,
          output: `🐳 沙箱已创建\n容器 ID: ${result.containerId}\n镜像: ${config.image}\n工作区挂载: ${config.mountWorkspace ? '是' : '否'}`,
          action: 'shell',
        }
  ...
  if (call.name === 'sandbox_exec') {
    ...
    const result = await execInContainer(call.arguments.container_id, call.arguments.command, {
      timeout: call.arguments.timeout,
    });
    return {
      success: result.success,
  ...
  if (call.name === 'sandbox_write') {
    ...
    return result.success
      ? {
          success: true,
          output: `已写入容器文件: ${call.arguments.path} (${Buffer.byteLength(call.arguments.content, 'utf-8')} bytes)`,
          action: 'write',
        }
  ...
  if (call.name === 'generate_image') {
    const result = await textToImage({
      ...
      savePath: call.arguments.save_path
        ? (safeResolvePath(ctx.workspacePath, call.arguments.save_path) ?? undefined)
        : undefined,
    });
    if (!result.success) {
  ...
  if (call.name === 'edit_image') {
    ...
    if (!srcBase64)
      return {
        success: false,
        output: `未找到标签为 "${call.arguments.image_label}" 的图像。请先截图或生成图像。`,
        action: 'web',
      };
  ...
  if (call.name === 'deploy_compose_up') {
    ...
    const result = await deployWithCompose({ projectName: call.arguments.project_name, services }, ctx.workspacePath, {
      buildFirst: call.arguments.build_first,
      detach: true,
    });
    return {
      success: result.success,
  ...
  if (call.name === 'deploy_compose_down') {
    ...
    return {
      success: result.success,
      output: result.success ? result.output : `停止失败: ${result.error}`,
      action: 'shell',
    };
  }
  ...
  if (call.name === 'deploy_health_check') {
    ...
    const lines = result.services.map(
      s => `${s.healthy ? '✅' : '❌'} ${s.name}: ${s.url} ${s.healthy ? `(${s.responseTime}ms)` : `— ${s.error}`}`,
    );
    return {
      success: result.success,
  ...
  if (call.name === 'git_create_branch') {
    ...
    return result.success
      ? {
          success: true,
          output: `已创建并切换到分支: ${call.arguments.branch_name}${call.arguments.base_branch ? ` (基于 ${call.arguments.base_branch})` : ''}`,
          action: 'git',
        }
  ...
  if (call.name === 'git_push') {
    const result = await gitPush(
      ctx.gitConfig,
      call.arguments.remote || 'origin',
      call.arguments.branch,
      call.arguments.set_upstream || false,
    );
    return result.success
  ...
  if (call.name === 'github_create_pr') {
    const pr = await createPR(
      ...
      call.arguments.title,
      call.arguments.body,
      call.arguments.head_branch,
      call.arguments.base_branch || 'main',
      call.arguments.draft || false,
    );
    return pr
      ? {
          success: true,
          output: `PR #${pr.number} 已创建: ${pr.html_url}\n${pr.draft ? '(草稿)' : ''}\n${pr.head_branch} → ${pr.base_branch}`,
          action: 'github',
        }
  ...
  if (call.name === 'github_list_prs') {
    ...
    const list = prs
      .map(
        p =>
          `#${p.number} [${p.state}${p.draft ? '/draft' : ''}] ${p.title} (${p.head_branch} → ${p.base_branch})${p.merged ? ' ✅merged' : ''}`,
      )
      .join('\n');
  ...
  if (call.name === 'github_merge_pr') {
    const result = await mergePR(
      ctx.gitConfig,
      call.arguments.pr_number,
      call.arguments.merge_method || 'squash',
      call.arguments.commit_title,
    );
    return result.success
      ? {
          success: true,
          output: `PR #${call.arguments.pr_number} 已合并 (${call.arguments.merge_method || 'squash'}) SHA: ${result.sha}`,
          action: 'github',
        }
  ...
  if (call.name.startsWith('supabase_')) {
    ...
    if (!accessToken) {
      return {
        success: false,
        output: '❌ Supabase 未配置: 缺少 supabase_access_token (请在密钥管理中添加)',
        action: 'web',
      };
    }
    ...
    if (call.name === 'supabase_status') {
      ...
      return status
        ? {
            success: true,
            output: `Supabase 项目状态:\n状态: ${status.status}\nAPI: ${status.apiUrl}\nDB: ${status.dbHost}\nAnon Key: ${status.anonKey ? status.anonKey.slice(0, 20) + '...' : '(未获取)'}`,
            action: 'web',
          }
    ...
    if (call.name === 'supabase_migration_create') {
      ...
      return {
        success: result.success,
        output: result.success ? `迁移文件已创建:\n${result.output}` : `创建失败: ${result.error}`,
        action: 'shell',
      };
    }
    ...
    if (call.name === 'supabase_db_pull') {
      ...
      return {
        success: result.success,
        output: result.success ? `Schema 拉取成功:\n${result.output}` : `拉取失败: ${result.error}`,
        action: 'shell',
      };
    }
    if (call.name === 'supabase_deploy_function') {
      ...
      return {
        success: result.success,
        output: result.success
          ? `Edge Function "${call.arguments.function_name}" 部署成功:\n${result.output}`
          : `部署失败: ${result.error}`,
        action: 'shell',
      };
    }
    if (call.name === 'supabase_gen_types') {
      ...
      return {
        success: result.success,
        output: result.success ? result.output : `类型生成失败: ${result.error}`,
        action: 'write',
      };
    }
  ...
  if (call.name.startsWith('cloudflare_')) {
    ...
    if (!apiToken || !accountId) {
      return {
        success: false,
        output: '❌ Cloudflare 未配置: 缺少 cloudflare_api_token 或 cloudflare_account_id (请在密钥管理中添加)',
        action: 'web',
      };
    }
    ...
    if (call.name === 'cloudflare_set_secret') {
      const ok = await cf.setWorkerSecret(
        cfConfig,
        call.arguments.worker_name,
        call.arguments.key,
      ...
      return ok
        ? {
            success: true,
            output: `Worker Secret "${call.arguments.key}" 已设置 (${call.arguments.worker_name})`,
            action: 'web',
          }
    ...
    if (call.name === 'cloudflare_dns_list') {
      if (!cfConfig.zoneId)
        return { success: false, output: '❌ 缺少 cloudflare_zone_id (请在密钥管理中添加)', action: 'web' };
      const records = await cf.listDNSRecords(cfConfig);
      if (records.length === 0) return { success: true, output: '无 DNS 记录', action: 'web' };
      const lines = records.map(
        r => `${r.type.padEnd(6)} ${r.name.padEnd(30)} → ${r.content} ${r.proxied ? '(proxied)' : ''}`,
      );
    ...
    if (call.name === 'cloudflare_dns_create') {
      ...
      return result.success
        ? {
            success: true,
            output: `DNS 记录已创建: ${call.arguments.type} ${call.arguments.name} → ${call.arguments.content} (ID: ${result.id})`,
            action: 'web',
          }
  ...
  if (call.name === 'run_command') {
    ...
    const isBackground = call.arguments.background === true || (call.arguments.timeout && call.arguments.timeout > 60);
    if (isBackground) {
    ...
    if (result.success) {
      ...
      return {
        success: false,
        output: `命令超时 (${Math.round(result.duration / 1000)}s):\n${result.stderr.slice(0, 2000)}`,
        action: 'shell',
      };
    } else {
      return {
        success: false,
        output: `命令失败 (exit ${result.exitCode}):\n${result.stderr.slice(0, 3000)}${result.stdout ? '\n--- stdout ---\n' + result.stdout.slice(0, 2000) : ''}`,
        action: 'shell',
      };
    }
  }
  ...
  if (call.name === 'run_lint') {
    ...
    return {
      success: result.success,
      output: `[run_lint] exit=${result.exitCode} duration=${result.duration}ms${result.timedOut ? ' TIMEOUT' : ''}\n${output.slice(0, 8000)}`,
      action: 'shell',
    };
  }
