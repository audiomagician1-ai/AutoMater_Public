...
contextBridge.exposeInMainWorld('automater', {
  ...
  // ── LLM ──
  llm: {
    testConnection: (provider: { type: string; baseUrl: string; apiKey: string }) =>
      ipcRenderer.invoke('llm:test-connection', provider),
    chat: (request: { model: string; messages: Array<{ role: string; content: string }> }) =>
      ipcRenderer.invoke('llm:chat', request),
    listModels: (provider: { type: string; baseUrl: string; apiKey: string }) =>
      ipcRenderer.invoke('llm:list-models', provider),
  },
  ...
  project: {
    ...
    getDesignDoc: (projectId: string) => ipcRenderer.invoke('project:get-design-doc', projectId),
    getDocChangelog: (projectId: string) => ipcRenderer.invoke('project:get-doc-changelog', projectId),
    // v4.4: 文档浏览器
    listAllDocs: (projectId: string) => ipcRenderer.invoke('project:list-all-docs', projectId),
    ...
    listChanges: (projectId: string) => ipcRenderer.invoke('project:list-changes', projectId),
    getImpactAnalysis: (changeRequestId: string) => ipcRenderer.invoke('project:get-impact-analysis', changeRequestId),
    // v5.1: 分析已有项目
    analyzeExisting: (projectId: string) => ipcRenderer.invoke('project:analyze-existing', projectId),
    // v7.0: Module Graph + Probe Analysis API
    getModuleGraph: (projectId: string) => ipcRenderer.invoke('project:get-module-graph', projectId),
    // v10.0: Architecture Tree (hierarchical)
    getArchTree: (projectId: string) => ipcRenderer.invoke('project:get-arch-tree', projectId),
    getKnownIssues: (projectId: string) => ipcRenderer.invoke('project:get-known-issues', projectId),
    getArchitectureDoc: (projectId: string) => ipcRenderer.invoke('project:get-architecture-doc', projectId),
    getProbeReports: (projectId: string) => ipcRenderer.invoke('project:get-probe-reports', projectId),
    getImportProgress: (projectId: string) => ipcRenderer.invoke('project:get-import-progress', projectId),
    ...
    getUserCorrections: (projectId: string) => ipcRenderer.invoke('project:get-user-corrections', projectId),
    // v16.0: 项目权限开关
    getPermissions: (projectId: string) => ipcRenderer.invoke('project:get-permissions', projectId),
    updatePermissions: (
      projectId: string,
  ...
  team: {
    ...
    /** v11.0: 测试成员级 LLM 连通性 */
    testMemberModel: (memberId: string, config: Record<string, unknown>) =>
      ipcRenderer.invoke('team:test-member-model', memberId, config),
  },
  ...
  workspace: {
    ...
    readFile: (projectId: string, relativePath: string) =>
      ipcRenderer.invoke('workspace:read-file', projectId, relativePath),
  ...
  // ── v2.0: 事件流 + Mission ──
  events: {
    query: (projectId: string, options?: Record<string, unknown>) =>
      ipcRenderer.invoke('events:query', projectId, options),
    getStats: (projectId: string) => ipcRenderer.invoke('events:get-stats', projectId),
    getTimeline: (projectId: string, featureId: string) =>
      ipcRenderer.invoke('events:get-timeline', projectId, featureId),
  ...
  secrets: {
    ...
    get: (projectId: string, key: string) => ipcRenderer.invoke('secrets:get', projectId, key),
    list: (projectId: string, provider?: string) => ipcRenderer.invoke('secrets:list', projectId, provider),
    delete: (projectId: string, key: string) => ipcRenderer.invoke('secrets:delete', projectId, key),
  },

  // ── v14.0: Issue Watcher (GitHub → Feature) ──
  issues: {
    sync: (projectId: string) => ipcRenderer.invoke('issues:sync', projectId),
    listFeatures: (projectId: string) => ipcRenderer.invoke('issues:list-features', projectId),
    startPolling: (projectId: string, intervalMinutes?: number) =>
      ipcRenderer.invoke('issues:start-polling', projectId, intervalMinutes || 10),
    stopPolling: (projectId: string) => ipcRenderer.invoke('issues:stop-polling', projectId),
  },
  ...
  session: {
    ...
    list: (projectId: string | null, agentId?: string) => ipcRenderer.invoke('session:list', projectId, agentId),
    listAll: (limit?: number) => ipcRenderer.invoke('session:list-all', limit),
    readBackup: (sessionId: string) => ipcRenderer.invoke('session:read-backup', sessionId),
    openBackupFolder: (sessionId: string) => ipcRenderer.invoke('session:open-backup-folder', sessionId),
    backupStats: () => ipcRenderer.invoke('session:backup-stats'),
    cleanup: (keepDays?: number) => ipcRenderer.invoke('session:cleanup', keepDays),
    ...
    sessionFeatures: (sessionId: string) => ipcRenderer.invoke('session:session-features', sessionId),
    ...
    batchFeatureSummaries: (projectId: string) => ipcRenderer.invoke('session:batch-feature-summaries', projectId),
    /** v22.0: 更新会话的聊天模式 */
  ...
  workflow: {
    ...
    create: (projectId: string, data: Record<string, unknown>) =>
      ipcRenderer.invoke('workflow:create', projectId, data),
    update: (presetId: string, updates: Record<string, unknown>) =>
      ipcRenderer.invoke('workflow:update', presetId, updates),
